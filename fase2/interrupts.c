...
static void _PLTHandler() {
    ...
    curr_process->p_s = *BLOCKED_PROCESS_STATE;
...
static void _ITHandler() {
    ...
    while (headBlocked(&device_semaphores[DEV_SEMAPHORES]) != NULL) {
        semV(&device_semaphores[DEV_SEMAPHORES], NULL, NULL);
        softBlock_count--;
    }
    device_semaphores[DEV_SEMAPHORES] = 0;
...
static void _terminalHandler(int device_number) {
    ...
    if (TERMINAL_STATUS(device_register->recv_status) == 5) { // Ricezione
        _deviceInterruptReturn(device_register->recv_status, &device_register->recv_command);
    }
    else if (TERMINAL_STATUS(device_register->transm_status) == 5) {  // Trasmissione
...
void interruptHandler() {
    ...
    unsigned int ip = BLOCKED_PROCESS_STATE->cause & CAUSE_IP_MASK;
